...
async function waitForStates(states, testRun, command) {
    ...
    while (!states.includes(newTestRun.status.toString())) {
        let { body: bodyTestRun } = await command.heroku.get(`/pipelines/${testRun.pipeline.id}/test-runs/${testRun.number}`);
...
async function display(pipeline, number, command) {
    ...
    if (testRun) {
        ...
        let { body: testNodes } = await command.heroku.get(`/test-runs/${testRun.id}/test-nodes`);
        ...
        if (testRun) {
            let { body: newTestNodes } = await command.heroku.get(`/test-runs/${testRun.id}/test-nodes`);
